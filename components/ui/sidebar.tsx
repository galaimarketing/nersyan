...
export const DesktopSidebar = ({
  ...
  return (
    <motion.div
      className={cn(
        "relative py-4 hidden md:flex md:flex-col flex-shrink-0 overflow-visible min-h-screen self-stretch",
...
export const MobileSidebar = ({
  ...
  return (
    <>
      <div
        className={cn(
          "h-10 px-4 py-4 flex flex-row md:hidden items-center justify-between bg-stone-100/90 dark:bg-neutral-800 w-full border-b border-stone-200/80 dark:border-neutral-700"
        )}
        {...props}
      >
...
export const SidebarLink = ({
  ...
  return (
    <Link
      href={link.href}
      className={cn(
        "flex items-center justify-start gap-3 group/sidebar py-2 px-2 rounded-lg min-h-[2.5rem]",
        className
      )}
      {...props}
    >
      ...
      <motion.span
        ...
        className="text-neutral-700 dark:text-neutral-200 text-sm group-hover/sidebar:translate-x-0.5 transition duration-150 whitespace-pre inline-block !p-0 !m-0"
      >
        {link.label}
      </motion.span>
    </Link>
